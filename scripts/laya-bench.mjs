...
const checkText = f => ({
  ...
  latency: ['Faster than the hosted call', `Median ${f.layaP50} on the laptop against ${f.jevP50} for the hosted call, including the network.`],
});
  ...
  return `<div class="bench-introduction laya-introduction"><h3>Then a decision you can own.</h3><p>A second bench, because this one compares two models rather than one rule on two trees: open weights on a laptop against a hosted API, on the same questions.</p></div>
      <div class="workbench laya-bench dark-surface" id="laya-bench">
        ...
        <div class="bench-body">
          ...
          <p class="bench-explainer">Result: the local port gives the same decision as the model’s reference code on <strong id="laya-parity">${escape(f.parity)}</strong> rows and is about ${f.speedup} times faster per call here, but it is ${escape(f.gapPoints)} points less accurate than Jev. That is beyond the ${refutation.accuracyMarginPoints}-point limit set before the run, so the accuracy check is <strong>${escape(f.verdicts.accuracy)}</strong> and stays that way.</p>
